<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(51, 51, 51, 1)","textAlign":"center"}' class="title">博文标题</view>
				<input :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"8rpx","color":"rgba(51, 51, 51, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"26rpx","borderStyle":"solid","height":"60rpx"}' :disabled="ro.bowenbiaoti" v-model="ruleForm.bowenbiaoti" placeholder="博文标题"></input>
			</view>
												<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(51, 51, 51, 1)","textAlign":"center"}' class="title">标签</view>
				<input :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"8rpx","color":"rgba(51, 51, 51, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"26rpx","borderStyle":"solid","height":"60rpx"}' :disabled="ro.biaoqian" v-model="ruleForm.biaoqian" placeholder="标签"></input>
			</view>
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(51, 51, 51, 1)","textAlign":"left"}' class="title">博文类型</view>
				<picker @change="bowenleixingChange" :value="bowenleixingIndex" :range="bowenleixingOptions">
					<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"8rpx","color":"rgba(51, 51, 51, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"26rpx","borderStyle":"solid","height":"60rpx"}' class="uni-input">{{ruleForm.bowenleixing?ruleForm.bowenleixing:"请选择博文类型"}}</view>
				</picker>
			</view>
		...
		async onLoad(options) {
			...
			this.user = res.data;
						...
						// ss读取
												...
												this.ruleForm.youxiang = this.user.youxiang
																																	
																								// 下拉框
			res = await this.$api.option(`bowenleixing`,`bowenleixing`,{});
			this.bowenleixingOptions = res.data;
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`bowenxinxi`, this.ruleForm.id);
			...
			if(options.cross){
				...
				for (var o in obj){
					if(o=='bowenbiaoti'){
					this.ruleForm.bowenbiaoti = obj[o];
					this.ro.bowenbiaoti = true;
					continue;
					}
					if(o=='biaoqian'){
					...
					if(o=='bowenleixing'){
					this.ruleForm.bowenleixing = obj[o];
					this.ro.bowenleixing = true;
					continue;
					}
					if(o=='tupian'){
		...
		methods: {
			...
			clicktimeConfirm(val) {
				...
				this.$forceUpdate();
																					...
																					// 下拉变化
			bowenleixingChange(e) {
				this.bowenleixingIndex = e.target.value
				this.ruleForm.bowenleixing = this.bowenleixingOptions[this.bowenleixingIndex]
			},
			...
			async onSubmitTap() {
																																																																																																																																																																																																																																																																																																																																																																																																																																																																												if((!this.ruleForm.bowenbiaoti)){
					this.$utils.msg(`博文标题不能为空`);
					return
				}
					...
					return
																																				...
																																				if(this.ruleForm.id){
					await this.$api.update(`bowenxinxi`, this.ruleForm);
				}else{
					await this.$api.add(`bowenxinxi`, this.ruleForm);
				}
